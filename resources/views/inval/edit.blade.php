    ...
    <div class="content mt-3">
        <div class="animated fadeIn">
            <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden p-6">
                ...
                <form action="{{ url('invalizin/' . $izin->id) }}" method="post" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="grid grid-cols-1 gap-6">
                            ...
                            <select name="guru_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                @foreach ($guru as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id == $izin->mengajar->guru_id ? 'selected' : '' }}>{{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="kelas_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                @foreach ($kelas as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id == $izin->mengajar->kelas_id ? 'selected' : '' }}>{{ $item->kelas }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="mapel_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                @foreach ($mapel as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id == $izin->mengajar->mapel_id ? 'selected' : '' }}>{{ $item->mapel }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" name="surat" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                            @if($izin->surat)
                        ...
                        <div>
                            ...
                            <input type="file" name="lampiran" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                            @if($izin->lampiran)

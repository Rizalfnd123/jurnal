    ...
    <div class="content mt-5">
        <div class="container mx-auto">
            <div class="max-w-4xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden mb-3">
                ...
                <div class="p-2">
                    <form action="{{ route('guru.import') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <input type="file" name="file" required
                                class="w-full sm:w-auto py-2 pl-2 text-sm text-gray-700 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-600" placeholder="pilih file">
                ...
                <div class="p-4">
                    <form action="{{ url('guru') }}" method="post" enctype="multipart/form-data" class="shadow-lg p-6 bg-gray-50 rounded-lg">
                        @csrf
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label for="nip" class="block text-gray-700 text-xs">NIP</label>
                                <input type="text" name="nip" id="nip" class="form-input mt-1 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-1" required autofocus>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="username" id="username" class="form-input mt-1 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-1" required>
                            </div>

                            <div>
                                <label for="password" class="block text-gray-700 text-xs">Password</label>
                                <input type="password" name="password" id="password" class="form-input mt-1 block w-full border border-gray-300 rounded-md shadow-sm text-xs p-1" required>
                            </div>

                            <div>
                                <label for="foto" class="block text-gray-700 text-xs">Foto</label>
                                <input class="block w-full mt-1 border border-gray-300 rounded-md shadow-sm text-xs p-1" type="file" name="foto" id="foto" onchange="previewImage(event)">
                                @if(isset($guru->foto))
